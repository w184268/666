...
class BridgeService : Service() {
    ...
    private fun notifyReady(item: Api.OutItem, filePath: String?) {
        ...
        val noti = NotificationCompat.Builder(this, CHANNEL)
            ...
            .setContentTitle(if (item.kind == "video") "视频已就绪" else "回复已就绪")
            .setContentText(text)
            ...
            .setAutoCancel(true)
            .apply {
                if (!filePath.isNullOrEmpty() && item.kind == "video") {
                    addAction(android.R.drawable.ic_menu_share, "发给孩子", pi)
                }
            }
            .build()
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (nm.getNotificationChannel(CHANNEL) == null) {
                nm.createNotificationChannel(
                    NotificationChannel(CHANNEL, "手表视频投递",
